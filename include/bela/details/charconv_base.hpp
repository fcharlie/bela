  ...
  case 10: { // Derived from _UIntegral_to_buff()
    ...
    if constexpr (_Use_chunks) { // For 64-bit numbers on 32-bit platforms, work
      ...
      while (_Value > 0xFFFF'FFFFU) { // ' TRANSITION, OS#21524983
        ...
        unsigned long _Chunk = static_cast<unsigned long>(
            _Value % 1'000'000'000); // ' TRANSITION, OS#21524983
        _Value = static_cast<_Unsigned>(
            _Value / 1'000'000'000); // ' TRANSITION, OS#21524983
...
_Digit_from_char(const wchar_t _Ch) noexcept {
...
[[nodiscard]] inline bool _Shift_left(_Big_integer_flt &_Xval,
  ...
  if (_Bit_shift == 0) {
    ...
    const uint32_t _New_used =
        _Xval._Myused + _Unit_shift +
        static_cast<uint32_t>(_Bit_shifts_into_next_unit);
    ...
    for (uint32_t _Dest_index = _New_used - 1; _Dest_index != _Unit_shift - 1;
         --_Dest_index) {
...
_Multiply_by_power_of_ten(_Big_integer_flt &_Xval,
  ...
  for (uint32_t _Large_power = _Power / 10; _Large_power != 0;) {
    const uint32_t _Current_power = _Min_value(
  ...
  static constexpr uint32_t _Small_powers_of_ten[9] = {
      10,           100,        1'000,
      10'000,       100'000, // ' TRANSITION, OS#21524983
      1'000'000,    10'000'000, 100'000'000,
      1'000'000'000}; // ' TRANSITION, OS#21524983
...
template <class _FloatingType>
[[nodiscard]] std::errc _Assemble_floating_point_value_t(
...
template <class _FloatingType>
[[nodiscard]] std::errc _Assemble_floating_point_value(
...
template <class _FloatingType>
[[nodiscard]] std::errc _Assemble_floating_point_value_from_big_integer_flt(
...
inline void _Accumulate_decimal_digits_into_big_integer_flt(
  ...
  for (const uint8_t *_It = _First_digit; _It != _Last_digit; ++_It) {
    if (_Accumulator_count == 9) {
      [[maybe_unused]] const bool _Success1 = _Multiply(
          _Result,
          1'000'000'000); // assumes no overflow // ' TRANSITION, OS#21524983
      assert(_Success1);
      [[maybe_unused]] const bool _Success2 =
          _Add(_Result, _Accumulator); // assumes no overflow
      assert(_Success2);
  ...
  if (_Accumulator_count != 0) {
    [[maybe_unused]] const bool _Success3 = _Multiply_by_power_of_ten(
        _Result, _Accumulator_count); // assumes no overflow
    assert(_Success3);
    [[maybe_unused]] const bool _Success4 =
        _Add(_Result, _Accumulator); // assumes no overflow
    assert(_Success4);
  }
}
...
_Convert_decimal_string_to_floating_type(const _Floating_point_string &_Data,
  ...
  const uint32_t _Positive_exponent =
      static_cast<uint32_t>(_Max_value(0, _Data._Myexponent));
  const uint32_t _Integer_digits_present =
      _Min_value(_Positive_exponent, _Data._Mymantissa_count);
  ...
  if (_Fractional_shift > 0) {
    [[maybe_unused]] const bool _Shift_success1 = _Shift_left(
        _Fractional_numerator, _Fractional_shift); // assumes no overflow
...
[[nodiscard]] std::errc _Convert_hexadecimal_string_to_floating_type(
    const _Floating_point_string &_Data, _FloatingType &_Result,
    bool _Has_zero_tail) noexcept {
  using _Traits = _Floating_type_traits<_FloatingType>;
...
_Ordinary_floating_from_chars(const wchar_t *const _First,
  ...
  // need to be strict.)
  constexpr ptrdiff_t _Maximum_adjustment =
      1'000'000; // ' TRANSITION, OS#21524983
  constexpr ptrdiff_t _Minimum_adjustment =
      -1'000'000; // ' TRANSITION, OS#21524983
  ...
  int _Exponent_adjustment = static_cast<int>(
      _Min_value(_Whole_end - _Leading_zero_end, _Maximum_adjustment));
  ...
  if (_Exponent_adjustment == 0) {
    ...
    _Exponent_adjustment =
        static_cast<int>(_Max_value(_Dot_end - _Next, _Minimum_adjustment));
  }
  ...
  if (_Exponent > _Maximum_temporary_decimal_exponent) {
    ...
    return {_Next,
            std::errc::result_out_of_range}; // Overflow example: "1e+9999"
  }

  if (_Exponent < _Minimum_temporary_decimal_exponent) {
    _Assemble_floating_point_zero(_Fp_string._Myis_negative, _Value);
    return {_Next,
            std::errc::result_out_of_range}; // Underflow example: "1e-9999"
  }
  ...
  if (_Exponent > _Maximum_temporary_decimal_exponent) {
    _Assemble_floating_point_infinity(_Fp_string._Myis_negative, _Value);
    return {_Next,
            std::errc::result_out_of_range}; // Overflow example: "10e+5199"
  }

  if (_Exponent < _Minimum_temporary_decimal_exponent) {
    _Assemble_floating_point_zero(_Fp_string._Myis_negative, _Value);
    return {_Next,
            std::errc::result_out_of_range}; // Underflow example: "0.001e-5199"
  }
  ...
  if (_Is_hexadecimal) {
    const std::errc _Ec = _Convert_hexadecimal_string_to_floating_type(
        _Fp_string, _Value, _Has_zero_tail);
    return {_Next, _Ec};
  } else {
    const std::errc _Ec = _Convert_decimal_string_to_floating_type(
        _Fp_string, _Value, _Has_zero_tail);
    return {_Next, _Ec};
  }
...
_Floating_from_chars(const wchar_t *const _First, const wchar_t *const _Last,
  ...
  assert(_Fmt == chars_format::general || _Fmt == chars_format::scientific ||
...
template <> struct _General_precision_tables<float> {
  ...
  static constexpr uint32_t _Special_X_table[63] = {
      ...
      0x3F7FFFFFu, 0x411FFFFFu, 0x42C7FFFFu, 0x4479FFFFu, 0x461C3FFFu,
      0x47C34FFFu, 0x497423FFu, 0x4B18967Fu};
...
_Floating_to_chars_general_precision(wchar_t *_First, wchar_t *const _Last,
                                     const _Floating _Value,
                                     int _Precision) noexcept {
  ...
  if (_Precision <= _Tables::_Max_special_P) {
    _Table_begin =
        _Tables::_Special_X_table + (_Precision - 1) * (_Precision + 10) / 2;
  ...
  const _Uint_type *const _Table_lower_bound = [=] {
    if constexpr (!std::is_same_v<_Floating, float>) {
      if (_Precision > 155) { // threshold determined via profiling
        return std::lower_bound(_Table_begin, _Table_end, _Uint_value,
                                std::less{});
      }
    }

    return std::find_if(_Table_begin, _Table_end, [=](const _Uint_type _Elem) {
      return _Uint_value <= _Elem;
    });
  }();
  ...
  const bool _Use_fixed_notation =
      _Precision > _Scientific_exponent_X && _Scientific_exponent_X >= -4;
  ...
  if (_Use_fixed_notation) {
    _Effective_precision = _Min_value(_Precision - (_Scientific_exponent_X + 1),
                                      _Max_fixed_precision);
    const to_chars_result _Buf_result = _Floating_to_chars_fixed_precision(
        _Buffer, std::end(_Buffer), _Value, _Effective_precision);
    _Significand_last = _Buf_result.ptr;
  } else {
    _Effective_precision =
        _Min_value(_Precision - 1, _Max_scientific_precision);
    const to_chars_result _Buf_result = _Floating_to_chars_scientific_precision(
        _Buffer, std::end(_Buffer), _Value, _Effective_precision);
  ...
  if (_Effective_precision > 0) {
    while (_Significand_last[-1] ==
           '0') { // will stop at '.' or a nonzero digit
      --_Significand_last;
    }
  ...
  const ptrdiff_t _Significand_distance =
      _Significand_last - _Significand_first;
  if (_Last - _First < _Significand_distance) {
    return {_Last, std::errc::value_too_large};
  }
  ArrayCopy(_First, _Significand_first,
...
_Floating_to_chars(wchar_t *_First, wchar_t *const _Last, _Floating _Value,
  ...
  if constexpr (_Overload == _Floating_to_chars_overload::_Plain) {
    assert(
        _Fmt ==
        chars_format{}); // plain overload must pass chars_format{} internally
  } else {
    assert(_Fmt == chars_format::general || _Fmt == chars_format::scientific ||
           _Fmt == chars_format::fixed || _Fmt == chars_format::hex);
  }
  ...
  if ((_Uint_value & _Traits::_Shifted_exponent_mask) ==
    ...
    if (_Mantissa == 0) {
      _Str = L"inf";
      _Len = 3;
    } else if (_Was_negative &&
               _Mantissa == _Traits::_Special_nan_mantissa_mask) {
      _Str = L"nan(ind)";
      _Len = 8;
