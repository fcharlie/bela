...
constexpr const interval zero_width[] = {
    ...
    {0xE0100, 0xE01EF}};
constexpr const interval double_width[] = {
...
bool bisearch(char32_t rune, const interval *table, size_t max) {
  size_t min = 0;
  size_t mid;
...
inline constexpr bool IsFullWidth(char32_t rune) {
  return (
      rune >= 0x1100 &&
      (rune <= 0x115f || /* Hangul Jamo init. consonants */
       rune == 0x2329 || rune == 0x232a ||
       (rune >= 0x2e80 && rune <= 0xa4cf && rune != 0x303f) || /* CJK ... Yi */
...
size_t CalculateWidth(char32_t rune) {
  using namespace unicode_internal;
  if (rune == 0) {
    return 0;
  }
  ...
  if (bisearch(rune, zero_width, std::size(zero_width) - 1)) {
    return 0;
  }
  // -------- faster
  if (rune < 0x1100) {
  ...
  if (bisearch(rune, double_width, std::size(double_width) - 1)) {
    return 2;
  }
  if (bisearch(rune, emoji_width, std::size(emoji_width) - 1)) {
    return 2;
  }
  if (IsFullWidth(rune)) {
    return 2;
