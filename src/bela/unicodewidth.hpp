#ifndef UNICODE_WIDTH_HPP
#define UNICODE_WIDTH_HPP

// Thansk https://github.com/fumiyas/wcwidth-cjk
// https://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c

namespace bela::codecvt_internal {
struct interval {
  char32_t first;
  char32_t last;
};
...
template <size_t N> bool bisearch(char32_t ch, const interval (&table)[N]) {
  size_t min = 0;
  size_t mid;
  auto max = N - 1;
...
inline bool IsFullWidth(char32_t ch) {
  return (ch >= 0x1100 &&
...
inline size_t CalculateWidthInternal(char32_t ch) {
  if (bisearch(ch, double_width) || bisearch(ch, emoji)) {
    return 2;
  }
  if (ch == 0) {
    return 0;
