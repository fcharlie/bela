...
static int64_t GetCurrentTimeNanosFromKernel(uint64_t last_cycleclock, uint64_t *cycleclock) {
  ...
  int loops = 0;
  do {
    before_cycles = base_internal::UnscaledCycleClockNow();
    current_time_nanos_from_system = base_internal::GetCurrentTimeNanosFromSystem();
    after_cycles = base_internal::UnscaledCycleClockNow();
    ...
    if (elapsed_cycles >= local_approx_syscall_time_in_cycles &&
      ...
      approx_syscall_time_in_cycles.store(local_approx_syscall_time_in_cycles, std::memory_order_relaxed);
    }
  ...
  if ((local_approx_syscall_time_in_cycles >> 1) < elapsed_cycles) {
    ...
    approx_syscall_time_in_cycles.store(new_approximation, std::memory_order_relaxed);
    seen_smaller.store(0, std::memory_order_relaxed);
  }
...
int64_t GetCurrentTimeNanos() {
  ...
  uint64_t now_cycles = base_internal::UnscaledCycleClockNow();
  ...
  if (seq_read0 == seq_read1 && (seq_read0 & 1) == 0 && delta_cycles < min_cycles_per_sample) {
...
static uint64_t UpdateLastSample(uint64_t now_cycles, uint64_t now_ns, uint64_t delta_cycles,
  ...
  if (sample->raw_ns == 0 || // no recent sample, or clock went backwards
      sample->raw_ns + static_cast<uint64_t>(5) * 1000 * 1000 * 1000 < now_ns || now_ns < sample->raw_ns ||
      now_cycles < sample->base_cycles) {
    ...
    if (new_nsscaled_per_cycle != 0 && diff_ns < 100 * 1000 * 1000 && -diff_ns < 100 * 1000 * 1000) {
      ...
      last_sample.nsscaled_per_cycle.store(new_nsscaled_per_cycle, std::memory_order_relaxed);
      uint64_t new_min_cycles_per_sample = SafeDivideAndScale(kMinNSBetweenSamples, new_nsscaled_per_cycle);
      last_sample.min_cycles_per_sample.store(new_min_cycles_per_sample, std::memory_order_relaxed);
      stats_calibrations++;
