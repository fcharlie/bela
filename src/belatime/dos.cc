...
constexpr int sinceWindowsEpochDays(int Y) {
  Y--;
  return Y * 365 + Y / 4 - Y / 100 + Y / 400 - EpochDays;
}
...
bela::Time FromDosDateTime(uint16_t dosDate, uint16_t dosTime) {
  ...
  auto mondays = leapYear ? LeapMonths[mon - 1] : Months[mon - 1];
  ...
  days += day - 1;
  return bela::FromUnixSeconds(days * dayPreSecond + hour * 3600 + minute * 60 + sec - UnixEpochStart);
}
} // namespace bela