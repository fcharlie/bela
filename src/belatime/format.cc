...
template <typename T> std::basic_string_view<T> trimSuffixZero(std::basic_string_view<T> sv) {
...
std::string DateTime::FormatNarrow(bool nano) {
  std::string stime;
  constexpr size_t bufsize = sizeof("2020-12-06T18:12:47.9858521+08:00") + 6;
  stime.reserve(bufsize);
  // Scratch buffer for internal conversions.
  char buf[3 + kDigits10_64]; // enough for longest conversion
  char *const ep = buf + sizeof(buf) / 2;
  ...
  stime.append(bp, static_cast<std::size_t>(ep - bp));
  if (nano) {
    bela::narrow::AlphaNum an(nsec);
    auto sv = trimSuffixZero(an.Piece());
    if (!sv.empty()) {
      stime.append(".").append(sv);
    }
  }
  if (tzoffset != 0) {
    stime.push_back((tzoffset > 0) ? '-' : '+');
